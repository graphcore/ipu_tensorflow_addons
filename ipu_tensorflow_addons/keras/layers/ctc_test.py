...
class CTCLossTest(test.TestCase, parameterized.TestCase):
  ...
  def testCTCPredictions(self, params):
    ...
    with strategy.scope():
      ...
      # IPU model
      predictions_layer_ipu = CTCPredictionsLayer(blank_index=blank_index,
                                                  from_logits=True,
                                                  beam_width=beam_width,
                                                  top_paths=top_paths)
  ...
  def testCTCInferenceLayerGetConfig(self):
    layer = CTCInferenceLayer(blank_index=0,
                              from_logits=True,
                              beam_width=10,
                              top_paths=1)
    config = layer.get_config()
    layer2 = CTCInferenceLayer.from_config(config)
    self.assertEqual(config, layer2.get_config())

  @test_util.run_v2_only
  def testCTCPredictionsLayerGetConfig(self):
    layer = CTCPredictionsLayer(blank_index=0, beam_width=10, top_paths=1)
    config = layer.get_config()
    layer2 = CTCPredictionsLayer.from_config(config)
    self.assertEqual(config, layer2.get_config())

  @test_util.run_v2_only
  def testCTCLossGetConfig(self):
    layer = CTCLoss(blank_index=0)
    config = layer.get_config()
    layer2 = CTCLoss.from_config(config)
    self.assertEqual(config, layer2.get_config())
